...
- r53 -> which type of policy would you choose given the scenario, in my case there were geo-proximity, geolocation and failover scenarions
...
- CloudHSM
- questions about ALB and NLB -> mostly questions about listeners, how to add SSL/TLS, which one should when we need if we want insane performances (NLB) or capabilities to inspect the request and based on it route it to a specific endpoint (ALB)
...
2. How to achieve lowest latency and highest networking capabilities between ec2 instances? -> cluster placement groups
3. SSM agent is installed on ec2 instances, but the instance is still not under the management of the SSM? -> we need to attach the policy to the EC2 instance
4. We want to track who accesses our ALB, what should we use? -> we can use the ALB's tracing and logging capabilities
5. How to get info on the memory utilization on EC2 instances as well as intervals as low as 1 minute? -> use CloudWatch agent and enable detailed monitoring (both)
6. If we do not want to hardcode the DB credentials in the code, where should we store it if we want to be able to rotate the credentials automatically and what do we need more to be able to make this solution work? -> Secret Manager and IAM ROLE attached to EC2/Lambda
7. A screenshot of the VPC Flow Log record and you are supposed to determine what went wrong ->
e.g:
...
8. We are using Lambda function with RDS and we get a "TooManyConnections" error, what should we do to resolve this issue -> use RDS proxy which will manage connection pools and clean up idle connections
9. A question regarding the AWS Cost Allocation Tags
10. How to connect an EC2 to s3 from VPC without traversing the public internet? -> VPC Gateway endpoint